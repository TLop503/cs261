...
void _adjustHeap(DynArr *heap, int max, int pos)
{
	...
	/* left, right, are children of pos*/
	int leftIdx;
	int rightIdx;
	int minIdx;
	...
	if (rightIdx <= max) {
		minIdx = _smallerIndexHeap(heap, leftIdx, rightIdx);

		/*if we need to swap*/
		if (compare(getDynArr(heap, minIdx), getDynArr(heap, pos)) == -1) {
			swapDynArr(heap, minIdx, pos);
...
void removeMinHeap(DynArr *heap)
{
	...
	/* -1 b/c we know last is good? need to check...*/
}

void _heapify(DynArr *input, int size, int i) {
	int lgst = i;
	int left = 2 * i + 1;
	int right = 2 * i + 2;
	int result;

	assert(left < sizeDynArr(input));
	result = compare(getDynArr(input, left), getDynArr(input, lgst));

	if (left < size && result == -1) {
		lgst = left;
	}

	assert(right < sizeDynArr(input));
	result = compare(getDynArr(input, right), getDynArr(input, lgst));
	if (right < size && result == -1) {
		lgst = right;
	}

	swapDynArr(input, lgst, i);
	if (lgst != i) {
		_heapify(input, size, lgst);
	}
}
